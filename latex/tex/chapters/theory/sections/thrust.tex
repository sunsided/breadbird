...
In axial climb, the overall velocity increases by an amount of $v_c$ (the climb velocity), that is
%
\begin{align}
v_{0_c} = v_0 + v_c \qquad
v_{i_c} = v_i + v_c \qquad
v_{3_c} = v_3 + v_c
\end{align}
%
...
%
\begin{align}
F_c &= \dot{m} \parens{ v_{3_c} - v_{0_c} } = \dot{m} \parens{ \parens{v_3 + v_c} - \parens{v_0 + v_c} } \\
    &= \dot{m} \parens{ v_3 - v_0 }
\end{align}
%
which is the same result as we obtained for the static case in \cref{eq:rotor_force_deltav} \vpageref{eq:rotor_force_deltav}.
Also, the induced velocity is now given as
%
\begin{align}
v_{i_c} &= \frac{1}{2} \parens{ v_{3_c} + v_{0_c} }
         = \frac{1}{2} \parens{ v_3 + v_0 + 2 v_c }
\end{align}
%
Given the work-energy principle we know that the work done by the rotor must equal the energy change in the air, that is
%
\begin{align}
W_{c} &= E(v_{3_c}) - E(v_{0_c}) \\
      &= \frac{1}{2} \dot{m} \parens{v_3 + v_c}^2 - \frac{1}{2} \dot{m} \parens{v_0 + v_c}^2
\end{align}
%
