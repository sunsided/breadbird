...
\begin{figure}
	...
	\caption[Mass flow rate and force]
			{Relation of the mass flow rate $\dot{m}$ acting on a disk of area $A = 2\pi r$ to the resulting force $\vec{F}$.}
...
\end{align}
%
where $v_0$ is the velocity of the actuator disk%
\footnote{Remind that $v_0$ can be understood as the actuator disk velocity as well as the free stream velocity, depending on the point of view.} 
and using \cref{eq:massflow,eq:newtons_2nd_mdot}, the force applied to the air in the outstream can be calculated \cite{durand1935} as
%
\begin{align}
F &= \dot{m} v_{rel} \notag \\
  &= \dot{m} \left( v_3 - v_0 \right) \\
  &= \rho A v_i \left( v_3 - v_0 \right) \label{eq:thrust_mdot}
\end{align}
%
