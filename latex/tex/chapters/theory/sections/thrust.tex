...
Using actuator disk concept, we assume that the area swept out by the rotating propeller blades describes an infinitesimally thin disk (the so-called \textit{rotor disk}) with orifice area $A = 2\pi r$, where
$r$ (in \withunit{\metre}) is the length of the propeller blades.
...
Since the momentum and energy induced by the rotor must be conserved,
the eventual outflow pressure $p_\infty$ and velocity $v_\infty$ can be used
to describe the force acting on the outflow through the rotor.

Given the relative speed of air in the outflow
%
\begin{align}
v_{rel} &= v_{\infty} - v_{self}
\end{align}
%
where $v_{self}$ is the velocity of the propeller engine itself\footnote{Instead 
of $v_{self}$, the \textit{free stream} velocity is generally used 
to more accurately describe the concept.}, 
and \name{Newton}'s
second law of motion for a constant velocity and variable mass flow rate \cite{book:siekmann2008}
%
\begin{align}
F &= \frac{\diff}{\diff t} \parens{ m v } \\
  &= \frac{\diff m}{\diff t} v = \dot{m} v \quad \text{if} \quad \sfrac{\diff v}{\diff t} = 0
\end{align}
%
the force applied to the air in the outstream can be calculated as \cite{durand1935}
%
\begin{align}
F &= \dot{m} v_{rel} \notag \\
  &= \dot{m} \left( v_{\infty} - v_{self} \right) \\
  &= \rho A v_i \left( v_{\infty} - v_{self} \right) \label{eq:thrust}
\end{align}
%
This essentially states that if the outflow velocity is larger than the velocity
of the accelerating system itself, the force applied on the air is positive, which
results in the thrust 
%
\begin{align}
\vec{F}_T &= -\vec{F}
\end{align}
