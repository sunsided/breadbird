...
\subsection{Momentum theory in hover state}
...
Using the actuator disk concept, we assume that the volume swept out by the rotating propeller blades describes an infinitesimally thin disk (the so-called \textit{rotor disk}) with orifice area $A = \pi r^2$%
...
Given the orifice area $A_r$ (the cross-section of the rotor disk), we can then define the volumetric flow rate % Volumenstrom
...
Since mass enters and leaves the control volume only through upstream and downstream orifice areas $A_u = \pi r^2_u$ and $A_d = \pi r^2_d$ (compare \cref{fig:streamtube_concept}), continuity equation gives that 
%
\begin{align}
...
Using \cref{eq:froude_rankine_theorem} in \cref{eq:rotor_force_deltav} we can now relate the force acting on the air to the induced velocity $v_i$, such that
%
\begin{align}
...
Since we have the thrust $T$ defined to be the reaction force to the action of moving mass, it consequently is given using \cref{eq:thrust_mdot} as\todo[noinline]{it's easier just to $(v_0 - v_3)$ instead}
%
\begin{align}
...
Furthermore, the induced velocity of the static case -- the \textit{hover velocity}\index{velocity!hover} $v_h$ -- can be trivially calculated from \cref{eq:static_thrust_Av} using $v_i = \sfrac{v_3}{2}$ (compare \cref{eq:froude_rankine_theorem})as
%
\begin{align}
F_{0} &= \frac{1}{2} \rho A_r (2 v_i)^2 \notag
       = 2 \rho A_r v_i^2 \\
v_h &\equiv v_{i_0} = \sqrt{\frac{F}{2 \rho A_r}}
\end{align}
...
\subsection{Momentum theory in axial climb and descent}

\subsubsection{Axial climb}

\todo{add fig. 3.17 from \cite{book:priha2006}}

In axial climb, by relativity of motion, the atmospheric velocity is equal to the amount of the climb velocity $\left|\vec{v}_c \right|$, that is
%
\begin{align}
v_c = v_0
\end{align}
%
Following from that, the thrust force $F_c$ is given as
%
\begin{align}
F_c &= \dot{m} \parens{ v_3 - v_c }
\end{align}
%
which is analog to \cref{eq:rotor_force_deltav} \vpageref{eq:rotor_force_deltav}, with an induced velocity of
%
\begin{align}
v_{i_c} &= \frac{1}{2} \parens{ v_3 + v_c }
\end{align}

Given the work-energy principle we know that the work done by the rotor must equal the energy change in the air, that is
%
\begin{align}
W_{c} &= E(v_c + \Delta v) - E(v_c) \\
      &= \frac{1}{2} \dot{m} \parens{v_c + \Delta v}^2 - \frac{1}{2} \dot{m} \parens{v_c}^2 \\
	  &= \frac{1}{2} \dot{m} \Delta v \parens{2 v_c + \Delta v}
\end{align}	  
with alternative solutions being
\begin{align}
W_{c} &= \frac{1}{4} \dot{m} \parens{v^2_3 - v^2_c} \\
 	  &= \frac{1}{4} \rho A_r \parens{v_3 + v_c} \parens{v^2_3 - v^2_c} \\
 	  &= \frac{1}{4} \rho A_r \parens{v_3 - v_c} \parens{v_3 + v_c}^2 \\
	  &= \frac{1}{2} \rho A_r \Delta v \, v_i^2
\end{align}
%
...
The major flaw with these equations is that they assume the induced velocity $v_i$ to be constant across the actuator disk and that no air mass leaves the control volume, as otherwise the assumption of momentum conservation would not hold.
In reality, this will not be generally true, as, for example, the induced velocity varies due to the shape of the propeller blades, most notably so at the blade tips and, of course, the hub\todo[noinline]{source}.
In order to model these effects, the thrust coefficient $c_T$ will be introduced later.

\todo{where is it introduced?}

\todo{add hint towards BET or BEMT}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% RELATION OF ROTOR SIZE TO OUTFLOW VELOCITY
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Relation between propeller size and outflow velocity}
\label{subsec:area_vs_velocity}
...
Generally, the equation for the kinetic energy\index{energy!kinetic} of a mass $m$ moving with velocity $v$ is given as
%
\begin{align}
E &= \frac{1}{2} m v^2
\end{align}
...
\Cref{fig:v3_A_energy} shows variations of outflow velocity $v_3$ and orifice area $A_r$ (in the range of $r_r = \SI{1}{\centi\metre} \dots \SI{10}{\centi\metre}$) for different thrust amounts and the resulting effect on the energy required.

\todo{note that larger propellers also mean larger rotational inertia and thus slower control response}

\begin{figure}
	\centering
	\input{pictures/mt_energy/mt_energy_3combined_linear.tikz}
	
	\rule{35em}{0.5pt}
	\caption[Outflow velocity and orifice area effects on energy consumption]{Relation of orifice area $A_r$ and outflow velocity $v_3$ to energy $E_{kin}$ required given a fixed amount of thrust $F$.}
	\label{fig:v3_A_energy}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MAXIMUM PROPELLER EFFICIENCY / BETZ LIMIT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Maximum propeller efficiency}

Enhancing on \name{Froude}'s and \name{Rankine}'s momentum theory, the maximum efficiency of a propeller is given as the \name{Lanchester}-\name{Betz}-\name{Joukowsky}\footnote{Although french spelling \name{Joukowsky} is used here, note that the transcription \name{Zhukovski} from the russian spelling is also commonly used \cite{WE:WE218}.} limit (discovered independently between 1915 and 1920, but generally attributed to \name{Betz} as the \name{Betz} limit \cite{WE:WE218}).
